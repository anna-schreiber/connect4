...
def evaluate_score(window, piece):
    ...
    if window.count(piece) == 4:
            ...
            score += 15 # Gives a score of 10 for 3-in-a-row
    elif window.count(piece) == 2 and window.count(0) == 2:
            score += 2 # Gives a score of 10 for 3-in-a-row
...
def score_position(board, piece):
    ...
    score += center_count * 3 # Give higher scores for pieces places in the center of the board
...
def is_terminal_node(board):
    # Return true if the move is a terminal/winning move, false otherwise
    return winning_move(board, PLAYER_PIECE) or winning_move(board, CPU_PIECE) or len(get_valid_locations(board)) == 0 
    ...
    if depth == 0 or is_terminal:
        if is_terminal:
            if winning_move(board, CPU_PIECE):
                return (None, 1000000000) # Return the best score and the column that produces it
            elif winning_move(board, PLAYER_PIECE):
                return (None, -1000000000)
            else: # Game is over,  no more valid moves
                return (None, 0)
        else: # Depth is 0
            return (None, score_position(board, CPU_PIECE))
    if maximizingPlayer:
        value = -math.inf
        ...
        for col in valid_locations:
            ...
            b_copy = board.copy()
            drop_piece(b_copy, row, col, CPU_PIECE)
...
while not game_over:
    if MOVE == 0:
        game_version = int(input("Would you like to play against a beginner AI or expert AI? Enter 0 for beginner, 1 for expert: "))
        MOVE = 1
    # Get player's input
    if turn == PLAYER:
